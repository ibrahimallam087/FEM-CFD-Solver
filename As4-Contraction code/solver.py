...
# Load mesh data
# =========================
nodes_data = np.loadtxt('Nodes.txt', skiprows=1)
elements_data = np.loadtxt('Elements.txt', skiprows=1)
...
for e in range(num_elem):
    ...
    for i in range(2):
        for j in range(2):
            ...
            # Jacobian
            J = np.vstack((dN_dxi, dN_deta)) @ np.column_stack((x, y))
...
# background color
plt.tricontourf(triang, psi, levels=100, cmap='jet')

# contour lines on top
contours = plt.tricontour(triang, psi, levels=40, colors='k', linewidths=0.5)

plt.colorbar()
plt.title("Streamlines (psi)")
plt.axis('equal')
# ===== velocity magnitude =====
plt.figure()
plt.tricontourf(triang, Vmag, levels=500)
plt.colorbar()
...
# ===== velocity vectors =====
plt.figure()
plt.quiver(x_all, y_all, u, v)
plt.title("Velocity Field")
plt.axis('equal')

# ===== divergence =====
plt.figure()
plt.tricontourf(triang, D, levels=100)
plt.colorbar()
